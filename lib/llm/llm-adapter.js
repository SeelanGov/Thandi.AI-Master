...
/**
 * GROQ Provider (Primary LLM for RAG)
 */
export class GroqProvider extends LLMProvider {
  constructor(config = {}) {
    ...
    this.model = config.model || process.env.GROQ_MODEL || 'llama-3.1-70b-versatile';
  ...
  async generateText(prompt, options = {}) {
    const apiCall = async (opts) => {
      ...
      console.log(`[GROQ] Making request to ${this.baseURL} with model ${this.model}`);
      ...
      const data = await response.json();
      console.log(`[GROQ] Response received, ${data.choices?.length || 0} choices`);
  ...
  estimateCost(inputTokens, outputTokens) {
    ...
    const inputCost = inputTokens * 0.0000005; // $0.50 per 1M tokens
    const outputCost = outputTokens * 0.0000015; // $1.50 per 1M tokens
    return {
      inputCost,
      outputCost,
      totalCost: inputCost + outputCost,

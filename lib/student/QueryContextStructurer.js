...
class QueryContextStructurer {
  ...
  /**
   * Build career priorities section (enhanced existing functionality)
   */
  buildCareerPrioritiesSection(profile) {
    const { careerInterests } = profile;
  ...
  /**
   * Build LLM instructions for optimal response generation
   */
  buildInstructions(profile) {
    ...
    // Career interest instructions
    if (profile.careerInterests.hasCareerInterests) {
      instructions.push('Prioritize the student\'s stated career interests while suggesting related alternatives.');
    }

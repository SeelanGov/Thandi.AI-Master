 ...
 * 
 * @typedef {import('./types.js').LLMVerificationResult} LLMVerificationResult
...
Return ONLY the JSON object, no additional text.`;
  ...
  _enhanceWithHallucinationDetection(parsed, input) {
    ...
    const sourceText = ragChunks
      .filter(chunk => chunk && chunk.chunk_text && typeof chunk.chunk_text === 'string')
    ...
    const draftNumbers = (typeof draftAnswer === 'string' ? draftAnswer.match(numberPattern) : null) || [];
    ...
    const draftInstitutions = (typeof draftAnswer === 'string' ? draftAnswer.match(institutionPattern) : null) || [];

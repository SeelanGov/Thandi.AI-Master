...
export class BiasDetector {
  ...
  identifyBiasPatterns(careers, profile) {
    ...
    // 5. Career interest weighting bias pattern
    const careerInterestBias = this.detectCareerInterestWeightingBias(careers, profile);
    if (careerInterestBias.detected) {
      patterns.push({
        type: 'career_interest_weighting_bias',
        description: careerInterestBias.description,
        severity: careerInterestBias.severity,
        evidence: careerInterestBias.evidence
      });
      
      corrections.push(...careerInterestBias.corrections);
      confidence += 0.3;
    }

    // Record pattern for trend analysis
    if (this.config.enablePatternTracking && profile) {
  ...
  detectCulturalStereotypes(careers, profile) {
    ...
    if (profile && profile.subjects?.some(s => /mathematics|math/gi.test(s))) {
    ...
    if (profile && profile.subjects?.some(s => /life sciences|biology|chemistry/gi.test(s))) {
  ...
  /**
   * Detect career interest weighting bias patterns
   * @param {Array} careers - Array of career objects
   * @param {Object} profile - Student profile
   * @returns {Object} - Career interest weighting bias analysis
   * @private
   */
  detectCareerInterestWeightingBias(careers, profile) {
    // Only analyze if student has stated career interests
    if (!profile || !profile.careerInterests?.hasCareerInterests) {
      return { detected: false };
    }

    const grade = profile.grade || 12;
    const careerInterests = profile.careerInterests.rawText.toLowerCase();
    const patterns = [];
    const corrections = [];
    let severity = 0;

    // Expected weighting based on grade
    const expectedPrimaryWeight = grade <= 11 ? 0.4 : 0.6;
    const expectedAlternativeWeight = grade <= 11 ? 0.6 : 0.4;

    // Analyze if recommendations match stated interests
    const interestMatchingCareers = careers.filter(career => {
      const title = (career.title || '').toLowerCase();
      const description = (career.description || '').toLowerCase();
      
      // Simple keyword matching for stated interests
      const interestKeywords = careerInterests.split(/\s+/).filter(word => word.length > 3);
      return interestKeywords.some(keyword => 
        title.includes(keyword) || description.includes(keyword)
      );
    });

    const interestMatchRatio = interestMatchingCareers.length / careers.length;
    const expectedRange = {
      min: expectedPrimaryWeight - 0.15,
      max: expectedPrimaryWeight + 0.15
    };

    // Detect over-weighting of career interests
    if (interestMatchRatio > expectedRange.max) {
      patterns.push('career_interest_over_weighting');
      severity = Math.max(severity, (interestMatchRatio - expectedRange.max) / (1 - expectedRange.max));
      
      corrections.push({
        type: 'reduce_interest_weighting',
        action: `Reduce career interest matching to ${Math.round(expectedPrimaryWeight * 100)}% for Grade ${grade}`,
        priority: 'medium'
      });
    }

    // Detect under-weighting of career interests
    if (interestMatchRatio < expectedRange.min) {
      patterns.push('career_interest_under_weighting');
      severity = Math.max(severity, (expectedRange.min - interestMatchRatio) / expectedRange.min);
      
      corrections.push({
        type: 'increase_interest_weighting',
        action: `Increase career interest matching to ${Math.round(expectedPrimaryWeight * 100)}% for Grade ${grade}`,
        priority: 'medium'
      });
    }

    // Detect lack of alternatives for younger grades
    if (grade <= 11 && interestMatchRatio > 0.7) {
      patterns.push('insufficient_exploration_alternatives');
      severity = Math.max(severity, (interestMatchRatio - 0.7) / 0.3);
      
      corrections.push({
        type: 'boost_exploration_alternatives',
        action: 'Increase alternative career suggestions for exploration phase students',
        priority: 'high'
      });
    }

    // Detect too many alternatives for Grade 12
    if (grade === 12 && interestMatchRatio < 0.4) {
      patterns.push('excessive_alternatives_for_decision_phase');
      severity = Math.max(severity, (0.4 - interestMatchRatio) / 0.4);
      
      corrections.push({
        type: 'focus_on_stated_interests',
        action: 'Increase focus on stated career interests for Grade 12 decision-making',
        priority: 'high'
      });
    }
    ...
    return {
      ...
      evidence: {
        grade,
        expectedPrimaryWeight: Math.round(expectedPrimaryWeight * 100),
        expectedAlternativeWeight: Math.round(expectedAlternativeWeight * 100),
        actualInterestMatchRatio: Math.round(interestMatchRatio * 100),
        interestMatchingCareers: interestMatchingCareers.length,
        totalCareers: careers.length,
        careerInterests: profile.careerInterests.rawText
      },
      corrections,
      description: detected ? 
        `Career interest weighting bias detected: ${patterns.join(', ')}` : 
        'No career interest weighting bias detected'
    };
  }

...
export async function matchCareersToProfile(profile, options = {}) {
  ...
  try {
    ...
    // ENHANCED: Apply subject-category prioritization
    const prioritizedCareers = applySubjectCategoryPrioritization(validCareers, profile);
    ...
    if (prioritizedCareers.length < 3) {
      console.log(`   ⚠️ WARNING: Only ${prioritizedCareers.length} prioritized careers found, triggering intelligent fallback system`);
      
      try {
        const selector = getFallbackSelector();
        const fallbackCareers = await selector.selectFallbacks(profile, prioritizedCareers, 5);
        ...
        try {
          const legacyFallbacks = await getFallbackCareers(profile, prioritizedCareers);
          const neededCount = Math.max(0, 3 - prioritizedCareers.length);
          const fallbacksToAdd = legacyFallbacks.slice(0, neededCount);
          
          finalCareers = [...prioritizedCareers, ...fallbacksToAdd];

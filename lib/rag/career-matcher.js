...
import { DiversityConfigManager } from './diversity-config-manager.js';
import { STEMBooster } from './stem-booster.js';
import { recordBiasSession } from './bias-monitoring-dashboard.js';
...
let diversityEnforcer = null;
let diversityConfigManager = null;
let stemBooster = null;
...
function getBiasDetector() {
  if (!biasDetector) {
    biasDetector = new BiasDetector({
      ...
      categoryDominanceThreshold: 0.6,
      enableLogging: false, // Disable for production
      enablePatternTracking: true,
      minCareersForAnalysis: 3
    });
  }
  return biasDetector;
}

function getDiversityEnforcer() {
  if (!diversityEnforcer) {
...
function getDiversityConfigManager() {
  if (!diversityConfigManager) {
    diversityConfigManager = new DiversityConfigManager({
      maxCategoryDominance: 0.6,
      minCategoryRepresentation: 0.15,
      targetCategoryCount: 3,
      qualityPreservationWeight: 0.7,
  ...
  return diversityConfigManager;
}

function getSTEMBooster() {
  if (!stemBooster) {
...
export async function matchCareersToProfile(profile, options = {}) {
  ...
  try {
    ...
    // ENHANCED: Apply bias mitigation (with feature flag)
    let biasCorrectedCareers = validatedCareers;
    if (isFeatureEnabled('bias_mitigation', featureContext)) {
      console.log('🔍 Applying bias mitigation...');
      
      try {
        const biasDetector = getBiasDetector();
        const diversityEnforcer = getDiversityEnforcer();
        const stemBooster = getSTEMBooster();
        
        // Step 1: Detect bias patterns
        const biasAnalysis = biasDetector.performComprehensiveBiasAnalysis(validatedCareers, profile);
        if (biasAnalysis.biasPatterns.length > 0) {
          console.log(`   🚨 Detected ${biasAnalysis.biasPatterns.length} bias pattern(s)`);
        }
        
        // Step 2: Apply STEM boosting for qualified students
        const stemBoostedCareers = stemBooster.applySTEMBoosting(validatedCareers, profile);
        
        // Step 3: Enforce diversity while preserving quality
        biasCorrectedCareers = diversityEnforcer.enforceDiversity(stemBoostedCareers);
        
        console.log(`   ✅ Bias mitigation complete: ${biasCorrectedCareers.length} careers`);
        
        // Log bias mitigation statistics
        const biasStats = {
          originalCount: validatedCareers.length,
          finalCount: biasCorrectedCareers.length,
          biasPatterns: biasAnalysis.biasPatterns.length,
          stemBoosts: stemBoostedCareers.filter(c => c.stemBoost).length,
          diversityScore: diversityEnforcer.analyzeDiversityState(biasCorrectedCareers).diversityScore
        };
        console.log('📊 Bias mitigation stats:', biasStats);
        
      } catch (biasError) {
        console.error('❌ Bias mitigation failed:', biasError);
        // Continue with original careers if bias mitigation fails
        biasCorrectedCareers = validatedCareers;
      }
    } else {
      console.log('📊 Bias mitigation disabled by feature flag');
    }
    ...
    // ENHANCED: Record bias mitigation session for monitoring dashboard
    if (isFeatureEnabled('bias_mitigation', featureContext)) {
      try {
        // Analyze final outcomes for monitoring
        const teachingCareers = biasCorrectedCareers.filter(career => 
          career.category?.toLowerCase().includes('education') ||
          career.title?.toLowerCase().includes('teacher') ||
          career.title?.toLowerCase().includes('educator')
        );
        
        const stemCareers = biasCorrectedCareers.filter(career =>
          career.stemBoost ||
          career.category?.toLowerCase().includes('technology') ||
          career.category?.toLowerCase().includes('engineering') ||
          career.category?.toLowerCase().includes('science') ||
          career.title?.toLowerCase().includes('engineer') ||
          career.title?.toLowerCase().includes('developer') ||
          career.title?.toLowerCase().includes('scientist')
        );
        
        const categories = [...new Set(biasCorrectedCareers.map(c => c.category).filter(Boolean))];
        const averageQuality = biasCorrectedCareers.reduce((sum, c) => sum + (c.similarity || 0), 0) / biasCorrectedCareers.length;
        
        // Record session data for monitoring
        recordBiasSession({
          ...
          biasAnalysis: {
            biasDetected: teachingCareers.length > (biasCorrectedCareers.length * 0.4),
            teachingBias: teachingCareers.length > (biasCorrectedCareers.length * 0.6),
            categoryDominance: categories.length < 2,
            stemBias: stemCareers.length < (biasCorrectedCareers.length * 0.2)
          },
          diversityStats: {
            correctionApplied: biasCorrectedCareers.length !== validatedCareers.length,
            qualityPreserved: averageQuality > 0.5,
            diversityImprovement: categories.length >= 2 ? 0.2 : 0,
            categoryBalanceAdjustments: Math.abs(biasCorrectedCareers.length - validatedCareers.length)
          },
          stemStats: {
            stemStudentIdentified: profile.subjects?.some(s => 
              /mathematics|math|physical sciences|physics/gi.test(s)
            ),
            boostsApplied: stemCareers.filter(c => c.stemBoost).length,
            averageBoostValue: stemCareers.filter(c => c.stemBoost).reduce((sum, c) => sum + (c.stemBoost || 0), 0) / Math.max(stemCareers.filter(c => c.stemBoost).length, 1)
          },
          performanceStats: {
            processingTime: duration
          },
          outcomes: {
            teachingPercentage: (teachingCareers.length / biasCorrectedCareers.length) * 100,
            stemPercentage: (stemCareers.length / biasCorrectedCareers.length) * 100,
            categoryCount: categories.length,
            diversityScore: (categories.length / Math.max(biasCorrectedCareers.length, 1)) * 100,
            qualityScore: averageQuality
          }
        });
        
        console.log('📊 Bias monitoring session recorded');
      } catch (monitoringError) {
        console.error('❌ Error recording bias monitoring session:', monitoringError);
        // Don't fail the request if monitoring fails
      }
    }
    
    return biasCorrectedCareers;

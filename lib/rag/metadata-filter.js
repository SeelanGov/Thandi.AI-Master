...
export class MetadataFilter {
  ...
  _checkTextPatterns(text) {
    ...
    if (titleMatch && this._isLikelyCareerTitle(titleMatch[1])) {
      // ENHANCED: Additional check to exclude obvious non-careers
      const potentialTitle = titleMatch[1].trim().toLowerCase();
      const invalidTitles = ['question', 'bursary', 'program', 'university', 'college', 'school', 'course', 'degree', 'diploma'];
      
      if (!invalidTitles.includes(potentialTitle)) {
        return {
          isValid: true,
    ...
    // ENHANCED: Check for non-career context first
    const nonCareerContext = ['question', 'bursary', 'program', 'university', 'college', 'course'];
    const hasNonCareerContext = nonCareerContext.some(context => firstPart.includes(context));
    
    if (!hasNonCareerContext) {

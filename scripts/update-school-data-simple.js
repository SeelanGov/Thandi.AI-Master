...
/**
 * Simple School Master Data Update Script
 * Updates school data using the correct database schema
 */
...
console.log('🔄 School Master Data Update (Simple)\n');
...
async function analyzeNewData(filePath) {
  ...
  try {
    ...
    console.log(`\n   ${isClean ? '✅' : '❌'} Dataset Quality: ${isClean ? 'CLEAN - No Primary Schools' : `CONTAINS ${primaryCount} PRIMARY SCHOOLS`}`);
...
async function transformSchoolData(schools) {
  ...
  const transformedSchools = schools.map(school => {
    // Map the new JSON structure to our simple database schema
    return {
  ...
  try {
    ...
    claimedSchools.forEach(claimedSchool => {
      ...
      if (existsInNew) {
        ...
        mergedSchools.push({
          school_id: claimedSchool.school_id,
          name: claimedSchool.name,
          province: claimedSchool.province,
          type: claimedSchool.type,
          status: claimedSchool.status,
          principal_email: claimedSchool.principal_email,
          contact_phone: claimedSchool.contact_phone,
          claimed_at: claimedSchool.claimed_at,
          claimed_by_school_uuid: claimedSchool.claimed_by_school_uuid

...
runCheck('No console.log in production files', () => {
  ...
  const acceptable = logCount <= 5;
  return { 
    passed: acceptable, 
    message: `${logCount} console.log statements found ${acceptable ? '(acceptable)' : '(too many for production)'}` 
  };
});
...
runCheck('P0 Issues Resolution', () => {
  ...
  const noPOPIA = !assessmentForm.includes('ConsentCheckbox') && !assessmentForm.includes('consent.given');
...
if (passRate >= 90) {
  ...
  console.log('🟡 STATUS: DEPLOYMENT WITH CAUTION');
